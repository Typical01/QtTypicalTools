...
namespace Typical_Tool {
	...
	namespace WindowsSystem {
		...
		class TYPICALTOOL_API Key {
		private:
			//
		public:

			static void wait_s(int64_t ms);

			/* 模拟键盘点击: 单击/按下/抬起
			* 间隔时间
			*/
			static void KeyClick(const BYTE& _bVk, const int64_t& _intervalTime = 10L, const ktm& _keyMode = ktm::ClickOne, void(*_func)() = [] {});

			/* 模拟鼠标点击: 鼠标事件(按键按下/抬起, 移动)
			* 间隔时间
			*/
			static void MouseClick(DWORD _mouseEvent, const int64_t& _intervalTime = 10L, void(*_func)() = [] {});
		...
		// WindowFont
		class TYPICALTOOL_API WindowFont {
		private:
			HFONT Font;
		...
		public:
			...
			template<class T = bool>
			void SetFont(HFONT hFont) { this->Font = hFont; }
		...
		public:
			RegisterHotKeyMessage(Tstr& RegisterHotKeyMessage, int32_t message)
			{
				Message = RegisterHotKeyMessage;
				if (message > 0) {
					LogDebug(LogTip, Printf(TEXT("热键注册[%s]: 成功!"), this->Message));
				}
				else {
					LogDebug(LogErr, Printf(TEXT("热键注册[%s]: 错误!\n\t代码(%s)"), this->Message, ToStr(message)));
				}
			}
			RegisterHotKeyMessage(Tstr&& RegisterHotKeyMessage, int32_t message)
			{
		...
		public:
			ShellMessage()
			{
				...
				ErrorCode = 0;
				Status = false;
				LogDebug(LogErr, Printf(TEXT("Shell消息[%s]: 非Shell错误()"), this->Message));
			}
			ShellMessage(const Tstr& ShellMessage, int32_t message)
			{
				...
				if (ErrorCode < 32) {
					...
					LogDebug(LogErr, Printf(TEXT("Shell消息[%s]: 错误!\n\t代码(%s)"), this->Message, ToStr(ErrorCode)));
				}
				else {
					this->Status = true;
					LogDebug(LogTip, Printf(TEXT("Shell消息[%s]: 成功!"), this->Message));
				}
			}

			//错误代码
			template<class T = bool>
			int32_t GetErrorCode() { return this->ErrorCode; }

			//是否成功
			template<class T = bool>
			bool IsSucceed() { return this->Status; }
		};
		...
		class WindowHost {
		public:
			UINT WM_TASKBARCREATED_WH = RegisterWindowMessage(TEXT("TaskbarCreated"));
		...
		private:
			inline static HINSTANCE hIns = nullptr;
			inline static int32_t hMenu = 1000; //自动分配的菜单ID
		...
		public:
			...
			template<class T = bool>
			bool AddWindowHost(Tstr windowName, HWND& window, int32_t showWindow = 5) {
				if (!IsWindow(window)) {
					//创建失败!
					lg(LogErr, Printf(TEXT("创建窗口失败!\n\t窗口名: [%s]"), windowName));
					return false;
				}
				LogDebug(LogTip, Printf(TEXT("创建窗口成功!\n\t窗口名: [%s]"), windowName));
		...
		public:
			...
			template<class T = bool>
			static int32_t RegisterWindowClass(WNDCLASS& wndClass) {
				if (!RegisterClass(&wndClass)) {
					lg(LogErr, Printf(TEXT("窗口类注册失败!\n\t窗口类名: [%s]"), wndClass.lpszClassName));
					return 0;
				}
				LogDebug(LogTip, Printf(TEXT("窗口类注册成功!\n\t窗口类名: [%s]"), wndClass.lpszClassName));
		...
		public:
			...
			void OutConfig()
			{
				LogDebug(LogTip, TEXT("ShellConfig::OutConfig()"));
				LogDebug(TEXT("菜单按钮: ") + this->MenuButton);
				LogDebug(TEXT("Shell操作: ") + this->ShellOperate);
				LogDebug(TEXT("文件: ") + this->File);
				LogDebug(TEXT("参数: ") + this->Arg);
				LogDebug(TEXT("显示窗口: ") + this->WindowShow);
		...
		public:
			...
			void ShellOperate(HMENU Menu, std::vector<ShellConfig>& _ShellConfig) {
				...
				for (auto tempShell = _ShellConfig.begin(); tempShell != _ShellConfig.end(); tempShell++) {
					...
					if (MenuButton == TEXT("否")) {
						...
						int32_t tempMenuID = WindowHost::GetHMENU();
						...
						if (AppendMenu(Menu, MF_STRING, tempMenuID, OperateName.c_str())) {
							tempShell->OutConfig(); //输出配置
							LogDebug(TEXT("  程序菜单项: 成功!"));
						}
						else {
							LogDebug(TEXT("  程序菜单项: 失败!"));
						}
					}
				}
			}
			...
			void ExeRunItemShell() {
				...
				if (ExeRunItem.size() > 0) {
					...
					lgcr(LogWar, TEXT("ExeRunItemShell: 没有执行项!"));
				}
			}
			//程序菜单项Shell
			template<class T = bool>
			void ExeMenuItemShell(int32_t _MenuItemID) {
				...
				if (tempIndex != ExeMenuItem.end()) {
					...
					lgcr(LogErr, Printf(TEXT("ExeMenuItemShell: 没有找到菜单选项 [%s]!"), _MenuItemID));
				}
			}
			...
			*/
			...
			static ShellMessage ExecuteAnalyze(Tstr OperateName, Tstr ShellOperate, Tstr ShellFile, Tstr ShellArg = TEXT(""), Tstr WindowShow = TEXT("是")) {
				if (ShellOperate == TEXT("打开文件") || ShellOperate == TEXT("open")) {
					...
					lgcr(LogWar, TEXT("ExecuteAnalyze: Shell操作模式错误(打开文件/打开文件夹/管理员运行)"));
					lgcr(LogWar, Printf(TEXT("ExecuteAnalyze: 操作名: [%s]"), OperateName));
					return ShellMessage();
				}
				...
				ShellMessage temp(OperateName, (int32_t)(int64_t)ShellExecute(NULL, ShellOperate.c_str(), ShellFile.c_str(), ShellArg.c_str(), NULL, ShowWindow));
				return temp;
			}
		};
		...
		*/
		...
		void SetDisplaySize(int32_t WidthValue, int32_t HeightValue, const Tstr& DisplayName = L"") {
			...
			// 枚举所有显示器
			for (int32_t i = 0; EnumDisplayDevices(NULL, i, &displayDevice, 0); i++) {
				...
				if (DisplayName.empty() || DisplayName == displayDevice.DeviceName) {
					...
					if (EnumDisplaySettings(displayDevice.DeviceName, ENUM_CURRENT_SETTINGS, &devMode)) {
						...
						if (result == DISP_CHANGE_SUCCESSFUL) {
							...
							LogDebug(LogTip, Printf(TEXT("分辨率已成功修改为: %ssx%ss"), ToStr(WidthValue), ToStr(HeightValue)));
						}
						else {
							LogDebug(LogErr, Printf(TEXT("无法修改分辨率，错误代码: %ss"), ToStr(result)));
						}
					}
					else {
						LogDebug(LogErr, TEXT("无法获取当前显示设置!"));
					}
			...
			// 如果指定了显示器名称但未找到
			if (!DisplayName.empty()) {
				LogDebug(LogErr, Printf(TEXT("未找到指定的显示器: %ss"), DisplayName));
			}
		}

		// 分辨率信息
		struct TYPICALTOOL_API Resolution {
			__int32 Width;
			__int32 Height;
		};
		...
		*/
		...
		void WindowDPI() {
			...
			if (SetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_SYSTEM_AWARE) == NULL) { //传入的值无效
				LogDebug(TEXT("Windows DPI: 传入的值无效."));
				return;
			}

			LogDebug(TEXT("Windows DPI: DPI感知(DPI_AWARENESS_CONTEXT_SYSTEM_AWARE) 设置成功!"));
#endif
		}

		//单一实例窗口程序
		template<class T = bool>
		int32_t AloneRun(Tstr windowClassName, Tstr windowTitleName) {
			//程序启动初始化
			HWND handle = FindWindow(windowClassName.c_str(), windowTitleName.c_str());
			if (handle != NULL) {
				lgr(LogWar, Printf(TEXT("应用程序已在运行: [%s]"), windowTitleName));
				return 0;
			}
			return 1;
		}
		...
		bool GainAdminPrivileges(Tstr strApp) {
			if (!IsUserAdmin()) { //非管理员权限, 则申请
				ShellMessage UserAdmin(TEXT("申请管理员权限"), (int32_t)ShellExecute(NULL, TEXT("runas"), strApp.c_str(), NULL, NULL, SW_SHOWNORMAL));
				//成功申请时, 退出当前进程
		...
		*/
		...
		bool SetSelfStarting(Tstr valueName, Tstr exePath, Tstr CommandLineParam, bool _bAutoStart) {
			...
			if (result != ERROR_SUCCESS) {
				LogDebug(LogErr, Printf(TEXT("打开密钥[%s]失败!"), ToStr(result)));
				return false;
			}
			...
			if (_bAutoStart) { // 如果需要设置为自启动
				...
				if (result == ERROR_FILE_NOT_FOUND || (result == ERROR_SUCCESS && exePath != existingValue)) {
					result = RegSetValueEx(hKey, valueName.c_str(), 0, REG_SZ, reinterpret_cast<const BYTE*>(exePath.c_str()), (exePath.size() + 1) * sizeof(Tchar));
					if (result != ERROR_SUCCESS) {
						LogDebug(LogErr, Printf(TEXT("设置注册表值[%s]失败!"), ToStr(result)));
						RegCloseKey(hKey);
						return false;
					}
					LogDebug(LogTip, TEXT("已[设置]开机自启动."));
				}
				else if (result != ERROR_SUCCESS) {
					LogDebug(LogErr, Printf(TEXT("设置: 查询注册表值[%s]失败!"), ToStr(result)));
					RegCloseKey(hKey);
					return false;
				}
			}
				...
				if (result == ERROR_SUCCESS) {
					...
					if (result != ERROR_SUCCESS) {
						LogDebug(LogErr, Printf(TEXT("删除注册表值[%s]失败!"), ToStr(result)));
						RegCloseKey(hKey);
						return false;
					}
					LogDebug(LogTip, TEXT("已[取消]开机自启动."));
				}
				else if (result != ERROR_FILE_NOT_FOUND) {
					LogDebug(LogErr, Printf(TEXT("取消: 查询注册表值[%s]失败!"), ToStr(result)));
					RegCloseKey(hKey);
					return false;
				}
			}
		...
		bool GetExePathName(Tstr& _ExeName) {
			...
			if (length > 0 && length < MAX_PATH) {
				LogDebug(Printf(TEXT("当前可执行文件的名称: [%s]"), _ExeName));
				return true;
			}
			else {
				LogDebug(TEXT("无法获取当前可执行文件的路径!"));
				return false;
			}
		}
		...
		//移动光标到目标位置
		TYPICALTOOL_API void MoveCursorLocation(int32_t x, int32_t y);
	}
