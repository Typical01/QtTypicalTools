...
namespace Typical_Tool {
	namespace time {
		using century = std::chrono::duration<int64_t, std::ratio<60LL * 60LL * 24LL * 7LL * 30LL * 12LL * 100LL, 1LL>>;	// 世纪
		using year = std::chrono::duration<int64_t, std::ratio<60LL * 60LL * 24LL * 7LL * 30LL * 12LL, 1LL>>;				// 年
		using month = std::chrono::duration<int64_t, std::ratio<60LL * 60LL * 24LL * 7LL * 30LL, 1LL>>;					// 月
		using week = std::chrono::duration<int64_t, std::ratio<60LL * 60LL * 24LL * 7LL, 1LL>>;							// 周
		using day = std::chrono::duration<int64_t, std::ratio<60LL * 60LL * 24LL, 1LL>>;									// 天
		using hour = std::chrono::duration<int64_t, std::ratio<60LL * 60LL, 1LL>>;										// 时
		using min = std::chrono::duration<int64_t, std::ratio<60LL, 1LL>>;												// 分
...
#define TimeToStr TimeMeasureToString
	...
	int64_t TransformTimes(const int64_t& TimePoint)
	{
	...
	Tstr TimeAutoToStr(int64_t _Time, TimeMeasure _Tm, bool _bShowMeasure = true, int32_t _FormatPlaceholder = 1, const Tstr& _TimeIntervalStr = TEXT(" "),
		...
		Tstr Result;
		std::map<TimeMeasure, int64_t> TimeStr; // 存储各时间单位对应的值
		std::vector<int64_t> Measures = { 1000, 1000, 1000, 60, 60, 24, 7, 4, 12, 100 }; // 转换关系

		// 从当前单位向更高级别单位转换
		for (int32_t i = _Tm; i < Measures.size(); ++i) {
			if (_Time >= Measures[i]) {
	...
	private:
		...
		void Time_IsValid_RunTime(int32_t _Number, const Tstr& _FunctionName)
		{
			if (this->IsSaveAllTimePoint) {
				_IsValid_RunTime<int32_t>(_Number, [&](int32_t _Number)->bool {
	...
	public:
		...
		template<typename T = bool>
		void SetTimer(const std::chrono::steady_clock::time_point& _TimePoint, int32_t _Location) {
		...
		template<typename T = bool>
		void SetTimer(std::chrono::steady_clock::time_point&& _TimePoint, int32_t _Location) {
			Time_IsValid_RunTime(_Location, TEXT("SetTimer()"));
		...
		template<typename T = bool>
		std::chrono::steady_clock::time_point GetTimer(int32_t _Location) {
	...
	public:
		...
		template<class Target = time::sec>
		int64_t ComputTime_FrontToBack()
		{
			// front - end
			return std::chrono::duration_cast<Target>(
		...
		template<class Target = time::sec>
		int64_t ComputTime(int32_t _LocationBegin, int32_t _LocationEnd)
		{
	...
	//时间
	class TYPICALTOOL_API Time {
	public:
		inline static bool IsShowLog = true;

	public:
		static void SetShowLog(bool _IsShowLog) { IsShowLog = _IsShowLog; }

		template<class Target = time::sec>
		static void sleep(int64_t _Number)
		{
			if (IsShowLog) {
				LogDebug(LogWar, Printf(TEXT("休眠: [%s]%s"), _Number, TimeMeasureToString<Target>()));
			}
			std::this_thread::sleep_for(Target(_Number));
		}
		template<class Target = time::sec>
		static void wait(int64_t _Number)
		{
			if (IsShowLog) {
				LogDebug(LogWar, Printf(TEXT("等待: [%s]%s"), _Number, TimeMeasureToString<Target>()));
			}
